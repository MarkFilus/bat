...
`gcd` and `lcm`. The corollary below gives the conceptual number-theoretic
content of the cyclic pseudoreflection formula used by the finite checker.
-/
...
/-- Complementary divisors are coprime exactly when the original divisors span `r`. -/
...
/-- Conceptual form of the primary cyclic smoothness test. -/
